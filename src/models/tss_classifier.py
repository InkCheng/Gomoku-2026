...
class TSSClassifier(nn.Module):
    ...
    def collect_training_data(self, state, result):
        """
                result:
                - 0: Win
                - 1: Draw
        ...
        self.training_data.append((state, one_hot_result))
...
class TSSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        state, label = self.data[idx]  # (state, int_label)
        state = torch.tensor(state, dtype=torch.float32)  # 确保是 Float Tensor
        label = torch.tensor(label, dtype=torch.long)  # 先转换为 long 类型索引
        label_one_hot = F.one_hot(label, num_classes=4).float()  # 变成 One-Hot 向量
        return state, label_one_hot  # 返回 one-hot label
